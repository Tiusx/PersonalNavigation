...
- 🏷️ **分类导航**：常用 / AI 工具 / 开发 / 学习 / 娱乐 / 购物 / 资讯，站点卡片式展示；登录后可**拖拽分类排序**（首页标签与设置面板均支持）
- 🎨 **图标灵活**：站点 / 分类 / 引擎图标支持**本地 SVG 品牌图标库**（SuperTinyIcons + thesvg，共 370+ 图标，可视化选择 + 搜索）与**图片 URL**，未设置时显示名称首字
- ⚡ **自动解析**：添加站点时点击「自动解析」即可**纯代码解析**网址——自动提取域名填入名称、匹配本地品牌图标库选中图标；未匹配到品牌图标时自动填入网站 favicon（仅拼 URL，不依赖任何第三方解析平台），结果由你确认后保存
- ✏️ **在线管理**：登录后添加 / 编辑 / 删除站点，管理分类（支持弹窗添加分类并选图标），实时生效
